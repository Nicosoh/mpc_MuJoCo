...
class InverseKinematicsSolver:
    ...
    def setup(self):
        ...
        self.configuration = pink.Configuration(model=self.robot.model, data=self.robot.data, q=self.robot.q0, collision_model=self.robot.collision_model, collision_data=self.robot.collision_data)
    ...
    def pad_yref(self):
        ...
        yref_vel = np.zeros_like(self.traj_qp)
        yref_u = np.zeros((self.traj_qp.shape[0], self.config["pin"]["nu"]))

        self.yref_pos_vel = np.hstack([self.traj_qp, yref_vel])
        self.yref_full = np.hstack([self.traj_qp, yref_vel, yref_u])
    ...
    def call_IK(self, yref):
        ...
        self.traj_qp = np.array(self.traj_qp) # Convert to np.array (only positions without velocity)

        sys.stdout.write("Trajectory of length: " + str(self.traj_qp.shape) + "\n")
