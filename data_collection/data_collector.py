...
def run_data_collector(model_name):
    ...
    for step in range(runs):
        ...
        save_summary(config, config_path="config.yaml", output_dir="data", sub_name=f"{model_name}_run")
        print(f"Completed data collection run {step+1}/{runs}")

    save_summary(data_config, elapsed=total_elapsed, config_path="data_collection/data_config.yaml", output_dir="data", sub_name=f"{model_name}_data_collection")

    # Save collected data
    save_npz(f"{model_name}_logs.npz", data=all_logs, output_dir="data")
